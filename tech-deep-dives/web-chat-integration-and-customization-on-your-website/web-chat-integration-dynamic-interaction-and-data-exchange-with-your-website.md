...
* `$answer` is triggered each time a user input leads through an answer on the platform. If there are [reroute](../../getting-started/agents-workflows-and-triggers/blocks/logic-blocks/reroute-block.md) blocks within an answer, multiple events are triggered.
